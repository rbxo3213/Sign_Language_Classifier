...
class SignLanguageRecognitionApp(QMainWindow):
    ...
    def __init__(self):
        ...
        # Load initial model
        self.current_model = "models/sign_language_classifier_cnn.tflite"
    ...
    def init_ui(self):
        ...
        # Current model label
        self.current_model_label = QLabel(f"Current Model: CNN", self)
        self.current_model_label.setAlignment(Qt.AlignCenter)
        self.current_model_label.setFont(QFont("Arial", 18))
        self.current_model_label.setStyleSheet("color: #333; margin-bottom: 20px;")

        # Model selection buttons
        self.model_buttons = {}
        model_paths = {
            "LSTM": "models/sign_language_classifier_lstm.tflite",
            "GRU": "models/sign_language_classifier_gru.tflite",
            "1D-CNN": "models/sign_language_classifier_cnn.tflite",
            "Bi-LSTM": "models/sign_language_classifier_bilstm.tflite",
            "Bi-GRU": "models/sign_language_classifier_bigru.tflite"
        }

        for model_name, model_path in model_paths.items():
            button = QPushButton(model_name, self)
            button.setFixedSize(150, 40)
            button.setFont(QFont("Arial", 14))
            button.setStyleSheet("background-color: #4CAF50; color: white; border: none; border-radius: 5px;")
            button.clicked.connect(lambda checked, path=model_path, name=model_name: self.change_model(path, name))
        ...
        for button in self.model_buttons.values():
            model_button_layout.addWidget(button)
    ...
    def load_model(self, model_path):
        self.fingerspelling_interpreter = tf.lite.Interpreter(model_path=model_path)
    ...
    def change_model(self, model_path, model_name):
        self.current_model = model_path
        self.load_model(self.current_model)
        self.result_label.setText(f"Model: {model_name}")
